...
func tagLiveSetTempo(data *XmlRoot, tags *tagger.Tagger) {
	...
	if data.LiveSet.MasterTrack.DeviceChain.Mixer.Tempo.Manual.Value > 0 {
		...
		if math.Trunc(data.LiveSet.MasterTrack.DeviceChain.Mixer.Tempo.Manual.Value) == data.LiveSet.MasterTrack.DeviceChain.Mixer.Tempo.Manual.Value {
			tags.AddSystemTag(fmt.Sprintf("bpm=%d", int(math.Round(data.LiveSet.MasterTrack.DeviceChain.Mixer.Tempo.Manual.Value))))
		} else {
			...
			tags.AddSystemTag(fmt.Sprintf("bpm=%d", int(math.Floor(data.LiveSet.MasterTrack.DeviceChain.Mixer.Tempo.Manual.Value))))
			tags.AddSystemTag(fmt.Sprintf("bpm=%d", int(math.Ceil(data.LiveSet.MasterTrack.DeviceChain.Mixer.Tempo.Manual.Value))))
		}
	}
}
...
func tagLiveSetTracks(data *XmlRoot, tags *tagger.Tagger) {
	...
	tags.AddSystemTag(fmt.Sprintf("ableton-live-set:tracks:count=%d", len(data.LiveSet.Tracks.AudioTracks)+len(data.LiveSet.Tracks.MidiTracks)))

	// Audio track specifics
	tags.AddSystemTag(fmt.Sprintf("ableton-live-set:audio-tracks:count=%d", len(data.LiveSet.Tracks.AudioTracks)))
	if len(data.LiveSet.Tracks.AudioTracks) > 0 {
		tags.AddSystemTag("ableton-live-set:audio-tracks:present=true")
	} else {
		tags.AddSystemTag("ableton-live-set:audio-tracks:present=false")
	}

	// Midi track specifics
	tags.AddSystemTag(fmt.Sprintf("ableton-live-set:midi-tracks:count=%d", len(data.LiveSet.Tracks.MidiTracks)))
	if len(data.LiveSet.Tracks.MidiTracks) > 0 {
		tags.AddSystemTag("ableton-live-set:midi-tracks:present=true")
	} else {
		tags.AddSystemTag("ableton-live-set:midi-tracks:present=false")
	}
}
...
func tagLiveSetPath(path string, tags *tagger.Tagger) {
	...
	if util.PathContainsFolder(path, "Live Recordings") {
		tags.AddSystemTag("file:location=ableton-live-recording")
	}
